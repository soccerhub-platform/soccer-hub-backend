...
public class LeadStateMachineConfig extends StateMachineConfigurerAdapter<LeadStatus, LeadEvent> {
    ...
    public void configure(StateMachineTransitionConfigurer<LeadStatus, LeadEvent> transitions)
        ...
        transitions
                ...
                .withExternal()
                    .source(LeadStatus.IN_PROGRESS)
                    .target(LeadStatus.CONTRACT_PENDING)
                    .event(LeadEvent.START_CONTRACT)
                .and()
                .withExternal()
                ...
                .and()
                .withExternal()
                    .source(LeadStatus.CONTRACT_PENDING)
                    .target(LeadStatus.PAYMENT_PENDING)
                    .event(LeadEvent.CONTRACT_ACTIVATED)
                .and()
                .withExternal()
                    .source(LeadStatus.PAYMENT_PENDING)
                    .target(LeadStatus.CONVERTED)
                    .event(LeadEvent.FIRST_PAYMENT_RECEIVED)
                .and()
                .withExternal()
                    .source(LeadStatus.TRIAL_SCHEDULED)
                    .target(LeadStatus.IN_PROGRESS)
